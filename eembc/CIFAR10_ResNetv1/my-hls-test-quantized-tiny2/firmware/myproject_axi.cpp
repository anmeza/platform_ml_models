...
void myproject_axi(
    ...
    hls::stream<layer18_t> out_local("output_1");

    #pragma HLS STREAM variable=in_local depth=10000
    #pragma HLS STREAM variable=out_local depth=10000
    ...
    for(unsigned i = 0; i < N_OUT / layer18_t::size; ++i) {
        layer18_t ctype = out_local.read();
        for(unsigned j = 0; j < layer18_t::size; j++) {
            bool last = (is_last && (i * layer18_t::size + j == N_OUT - 1)) ? true : false;
            out[i * layer18_t::size + j] = output_axi_t(ctype[j], last);
        }
    }
}
